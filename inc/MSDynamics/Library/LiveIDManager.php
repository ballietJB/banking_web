...
class LiveIDManager {
    ...
    public function authenticateWithLiveID($CRMUrl, $liveIDUsername, $liveIDPassword, $URNAddress_, $is365 = true) {
	...
	if($is365)	
            $binaryDATokenXML = LiveIDManager::GetSOAPResponse("/extSTS.srf" , "login.microsoftonline.com" , "https://login.microsoftonline.com/extSTS.srf", $soapTemplate);
        else
            $binaryDATokenXML = LiveIDManager::GetSOAPResponse("/extSTS.srf" , "login.microsoftonline.com" , "https://login.microsoftonline.com/extSTS.srf", $soapTemplate);
	...
	$cipherValue =  $matches[1];
		...
		//para saber q foi este fui a XRMServices/2011/Organization.svc?wsdl=wsdl0 e depois no xml q devolveu:  <ms-xrm:AppliesTo>urn:crmemea:dynamics.com</ms-xrm:AppliesTo> 
		$URNAddress = $URNAddress_; 
